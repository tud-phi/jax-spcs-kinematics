...
black:
	black --version
	black --config pyproject.toml examples spcs_kinematics tests

.PHONY: black-check
black-check:
	black --version
	black --diff --check --config pyproject.toml examples spcs_kinematics tests

.PHONY: flake8
flake8:
	flake8 --version
	flake8 spcs_kinematics tests
...
.PHONY: test_coverage
test_coverage:
	pytest --cov=spcs_kinematics

.PHONY: test_coverage_xml
test_coverage_xml:
	pytest --cov=spcs_kinematics --cov-report=xml
